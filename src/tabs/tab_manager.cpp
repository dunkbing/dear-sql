...
void TabManager::closeTab(const std::string &name)
{
    auto it = std::find_if(tabs.begin(), tabs.end(),
                           [&name](const std::shared_ptr<Tab> &tab)
                           {
...
std::shared_ptr<Tab> TabManager::findTableTab(const std::string &databasePath, const std::string &tableName) const
{
    for (auto &tab : tabs)
    {
        if (tab->getType() == TabType::TABLE_VIEWER)
        {
            ...
            if (tableTab &&
                tableTab->getDatabasePath() == databasePath &&
...
std::shared_ptr<Tab> TabManager::createTableViewerTab(const std::string &databasePath, const std::string &tableName)
{
